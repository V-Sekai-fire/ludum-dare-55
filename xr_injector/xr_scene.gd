...
@onready var xr_main_viewport2d_in_3d_subviewport: SubViewport = xr_main_viewport2d_in_3d.get_node(
	"Viewport"
)
@onready
var xr_secondary_viewport2d_in_3d: Node3D = xr_camera_3d.get_node("XRSecondaryViewport2Din3D")
@onready
var xr_secondary_viewport2d_in_3d_subviewport: SubViewport = xr_secondary_viewport2d_in_3d.get_node(
	"Viewport"
)
...
@onready
var left_gesture_detection_area: Area3D = xr_left_controller.get_node("GestureDetectionArea")
@onready
var right_gesture_detection_area: Area3D = xr_right_controller.get_node("GestureDetectionArea")
...
@onready
var xr_physical_movement_controller: Node = xr_origin_3d.get_node("XRPhysicalMovementController")
...
@onready
var xr_reparenting_node_holder: Node3D = xr_reparenting_node.get_node("XRReparentingNodeHolder")
...
enum XR_RADIAL_TYPE { GAMEPAD = 0, KEYBOARD = 1, ACTION = 2 }
var xr_radial_menu_mode: XR_RADIAL_TYPE = XR_RADIAL_TYPE.GAMEPAD
var xr_radial_menu_entries: Array = [
	"Joypad Y/Triangle", "Joypad B/Circle", "Joypad A/Cross", "Joypad X/Square"
]
var open_radial_menu_button: String = "by_button"

# Decacis Stick Turning Variables
enum TurningType { SNAP = 0, SMOOTH = 1, NONE = 2 }
...
enum XR_VIEWPORT_LOCATION { CAMERA = 0, PRIMARY_CONTROLLER = 1, SECONDARY_CONTROLLER = 2 }
...
func _ready() -> void:
	...
	xr_physical_movement_controller.connect(
		"tree_exiting", Callable(self, "_on_xr_origin_exiting_tree")
	)
...
func _process(_delta: float) -> void:
	...
	if (
		!is_instance_valid(xr_left_controller)
		or !is_instance_valid(xr_right_controller)
		or use_physical_gamepad_only
	):
		return
	...
	if !is_instance_valid(current_roomscale_character_body) and use_roomscale == true:
		...
		if is_instance_valid(current_roomscale_character_body) and xr_origin_reparented == false:
			...
			if attempt_to_use_camera_to_set_roomscale_height:
				...
				var err = xr_roomscale_controller.set_enabled(
					true,
					xr_origin_3d,
					reverse_roomscale_direction,
					current_camera,
...
func map_xr_controllers_to_action_map() -> bool:
	...
	if primary_controller_selection.to_lower() == "right":
		...
		print(
			"Error: primary_controller_selection in control config file is neither set to right nor left, defaulting to right."
		)
		primary_controller = xr_right_controller
		primary_detection_area = right_gesture_detection_area
		secondary_controller = xr_left_controller
	...
	if stick_turn_controller.to_lower() == "primary_controller":
		...
		secondary_controller.connect(
			"input_vector2_changed", Callable(self, "secondary_stick_moved")
		)
...
func handle_primary_xr_inputs(button):
	...
	if (
		button == pointer_gesture_toggle_button
		and gesture_area.overlaps_area(primary_detection_area)
	):
		xr_pointer.set_enabled(!xr_pointer.enabled)

	# (Temporary) Set user height if user presses designated button while doing gesture
	if (
		button == gesture_set_user_height_button
		and gesture_area.overlaps_area(primary_detection_area)
	):
	...
	if (
		button == gesture_toggle_active_camera_button
		and gesture_area.overlaps_area(primary_detection_area)
	):
...
func handle_secondary_xr_inputs(button):
	...
	if (
		button == gesture_load_action_map_button
		and gesture_area.overlaps_area(secondary_detection_area)
	):
		xr_config_handler.load_action_map_file(xr_config_handler.game_action_map_cfg_path)
		if use_arm_swing_jump:
			xr_physical_movement_controller.detect_game_jump_action_events()
...
func process_joystick_inputs():
	...
	if (
		stick_emulate_mouse_movement
		and (
			abs(primary_x_axis.axis_value) > emulated_mouse_deadzone
			or abs(primary_y_axis.axis_value) > emulated_mouse_deadzone
		)
	):
		var mouse_move = InputEventMouseMotion.new()
		mouse_move.relative = (
			Vector2(primary_x_axis.axis_value, primary_y_axis.axis_value)
			* emulated_mouse_sensitivity_multiplier
		)
	...
	if current_controller == null or current_controller == c_controller:
		if stick_name == "primary":
			...
			if last_stick_val.length() < DEADZONE:
				...
				last_stick_val = (
					last_stick_val.normalized()
					* ((last_stick_val.length() - DEADZONE) / (1 - DEADZONE))
				)
...
func handle_node_reparenting(
	delta: float, reparented_node: Node3D, rotate_reparented_node_180_degrees: bool
):
	...
	if xr_reparenting_active and is_instance_valid(reparented_node):
		...
		if secondary_controller.is_button_pressed("grip"):
			if (
				(
					(
						secondary_controller.global_transform.origin
						- primary_controller.global_transform.origin
					)
					. length()
				)
				<= 0.35
		...
		if xr_two_handed_aim == true:
			...
			var dir = (
				(primary_controller.global_position - secondary_controller.global_position)
				. normalized()
			)
			if rotate_reparented_node_180_degrees:
				dir = (
					(secondary_controller.global_position - primary_controller.global_position)
					. normalized()
				)
			xr_reparenting_node.global_transform.basis = (
				xr_reparenting_node.global_transform.basis.looking_at(dir, Vector3.UP, true)
			)
		else:
		...
		handle_reparented_node_smoothing(
			delta, xr_reparenting_node_holder, reparented_node, rotate_reparented_node_180_degrees
		)
...
func handle_reparented_node_smoothing(
	delta: float,
	source_node: Node3D,
	destination_node: Node3D,
	rotate_reparented_node_180_degrees: bool
):
	if is_instance_valid(source_node) and is_instance_valid(destination_node):
		...
		if rotate_reparented_node_180_degrees:
			...
			last_pose_rotation = Basis(
				last_pose_rotation.slerp(new_pose_rotation, lerp_speed * spherical_distance)
			)
...
func _on_xr_started():
	...
	loaded = xr_config_handler.load_game_control_map_cfg_file(
		xr_config_handler.game_control_map_cfg_path
	)
...
func _on_xr_origin_exiting_tree():
	if (
		use_roomscale
		and is_instance_valid(current_roomscale_character_body)
		and xr_origin_reparented
	):
		...
		if attempt_to_use_camera_to_set_roomscale_height:
			...
			@warning_ignore("unused_variable")
			var err = xr_roomscale_controller.set_enabled(
				false, null, reverse_roomscale_direction, null, roomscale_height_adjustment
			)
...
func _setup_new_xr_origin(new_origin: XROrigin3D):
	...
	if xr_main_viewport_location == XR_VIEWPORT_LOCATION.CAMERA:
		...
		xr_main_viewport2d_in_3d = primary_controller.get_node("XRViewportHolder").get_node(
			"XRMainViewport2Din3D"
		)
	elif xr_main_viewport_location == XR_VIEWPORT_LOCATION.SECONDARY_CONTROLLER:
		xr_main_viewport2d_in_3d = secondary_controller.get_node("XRViewportHolder").get_node(
			"XRMainViewport2Din3D"
		)
	...
	if xr_secondary_viewport_location == XR_VIEWPORT_LOCATION.CAMERA:
		...
		xr_secondary_viewport2d_in_3d = primary_controller.get_node("XRViewportHolder").get_node(
			"XRSecondaryViewport2Din3D"
		)
	elif xr_secondary_viewport_location == XR_VIEWPORT_LOCATION.SECONDARY_CONTROLLER:
		xr_secondary_viewport2d_in_3d = secondary_controller.get_node("XRViewportHolder").get_node(
			"XRSecondaryViewport2Din3D"
		)
	...
	# Reset other nodes
	xr_physical_movement_controller.set_enabled(
		use_jog_movement,
		use_arm_swing_jump,
		primary_controller,
...
func setup_viewports():
	...
	print(
		"static body viewport before rewrite: ",
		xr_main_viewport2d_in_3d.get_node("StaticBody3D")._viewport
	)
	xr_main_viewport2d_in_3d.get_node("StaticBody3D")._viewport = get_viewport()
	print(
		"static body viewport after rewrite: ",
		xr_main_viewport2d_in_3d.get_node("StaticBody3D")._viewport
	)
	...
	xr_main_viewport2d_in_3d.set_viewport_size(
		xr_standard_viewport_size * primary_viewport_size_multiplier
	)
	xr_secondary_viewport2d_in_3d.set_viewport_size(
		xr_standard_viewport_size * secondary_viewport_size_multiplier
	)
...
func reparent_viewport(viewport_node, viewport_location):
	...
	if viewport_location == XR_VIEWPORT_LOCATION.CAMERA and viewport_parent == xr_camera_3d:
		...
		viewport_location == XR_VIEWPORT_LOCATION.PRIMARY_CONTROLLER
		and viewport_parent.get_parent() == primary_controller
	):
		return
	elif (
		viewport_location == XR_VIEWPORT_LOCATION.SECONDARY_CONTROLLER
		and viewport_parent.get_parent() == secondary_controller
	):
		return
...
func find_and_set_active_camera_3d():
	...
	for camera in cameras:
		if camera != xr_camera_3d:
			...
			if (
				...
				and camera.get_viewport() == xr_camera_3d.get_viewport()
				and current_roomscale_character_body == null
				and not manually_set_camera
			):
	...
	if (
		...
		for available_camera in available_cameras:
			var available_camera_remote_transform = available_camera.find_child(
				"*XRRemoteTransform", false, false
			)
...
func find_and_set_canvas_layers():
	var potential_canvas_layer_nodes: Array = get_node("/root").find_children(
		"*", "CanvasLayer", true, false
	)
	...
	if potential_canvas_layer_nodes != []:
		for canvas_layer in potential_canvas_layer_nodes:
			if (
				...
				and not canvas_layer.is_in_group("active_canvas_layers")
			):
	...
	if is_instance_valid(current_camera):
		...
		if is_instance_valid(potential_character_body_node):
			if !potential_character_body_node.is_class("CharacterBody3D"):
				...
				if (
					is_instance_valid(potential_character_body_node)
					and !potential_character_body_node.is_class("CharacterBody3D")
				):
					print(
						"parent of parent of current camera is not CharacterBody3D, ending simple search."
					)
					potential_character_body_node = null
					var potential_character_bodies: Array = get_node("/root").find_children(
						"*", "CharacterBody3D", true, false
					)
					...
					if potential_character_bodies.size() == 1:
						...
						for body in potential_character_bodies:
							if body.is_ancestor_of(current_camera):
								...
								or body.name.to_lower().contains("controller")
							):
								print(
									"Winning characterbody from recursive search with name search found: ",
									body
								)
								return body
				# If we have a valid node, and it was a class character body, we have our characterbody3D
					...
					print(
						"Character body found as parent of parent of current camera, sending to roomscale node: ",
						potential_character_body_node
					)
			...
			# If we have a valid node and it's a character body, we found our characterbody as a parent of the camera
				...
				print(
					"Character body found as parent to current camera, sending to roomscale node: ",
					potential_character_body_node
				)
				return potential_character_body_node
...
func set_worldscale_for_xr_nodes(new_xr_world_scale):
	...
	left_gesture_detection_area.get_node("ControllerGestureShape").shape.radius = (
		0.2 * new_xr_world_scale
	)
	left_gesture_detection_area.get_node("ControllerGestureShape").shape.margin = (
		0.04 * new_xr_world_scale
	)
	right_gesture_detection_area.get_node("ControllerGestureShape").shape.radius = (
		0.2 * new_xr_world_scale
	)
	right_gesture_detection_area.get_node("ControllerGestureShape").shape.margin = (
		0.04 * new_xr_world_scale
	)
...
func _set_vostok_gun(delta):
	if (
		is_instance_valid(xr_roomscale_controller)
		and is_instance_valid(xr_roomscale_controller.camera_3d)
	):
		var vostok_weapons_node = xr_roomscale_controller.camera_3d.find_child(
			"Weapons", false, false
		)
		if vostok_weapons_node != null:
			if vostok_weapons_node.get_child_count(true) > 0:
				...
				#print(vostok_weapon)
				var vostok_weapon_mesh = vostok_weapon.get_node(
					"Handling/Sway/Noise/Tilt/Impulse/Recoil/Weapon"
				)
				...
				if is_instance_valid(vostok_weapon_mesh):
					...
					if vostok_arms:
						...
						handle_node_reparenting(
							delta, vostok_weapon_mesh, rotate_reparented_node_180_degrees
						)
...
func _set_beton_gun(delta: float):
	var gun_node = get_tree().get_root().get_node_or_null(
		"LowresRoot/LowResViewport/Player/RotPoint"
	)
...
func _set_tar_object_picker(delta: float):
	if is_instance_valid(xr_roomscale_controller.current_characterbody3D):
		var object_picker_point = xr_roomscale_controller.current_characterbody3D.get_node_or_null(
			"RotationHelper"
		)
		if is_instance_valid(object_picker_point) and xr_origin_reparented:
			object_picker_point.get_node("PlayerEyes/obj_picker_point").transform.origin = Vector3(
				0, 0, 0
			)
			xr_reparenting_active = true
			var rotate_reparented_node_180_degrees = false
